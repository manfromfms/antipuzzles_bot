...
def E(mu, mu_, phi_):
    return 1 / (1 + math.exp(-g(phi_)*(mu-mu_)))


def f(x, delta, phi, v, sigma):
    a = math.log(sigma**2)
    return math.exp(x) * (delta**2 - phi**2 - v - math.exp(x)) / 2 / (phi**2+v**2+math.exp(x)) - (x-a) / tau**2
...
def calculate_rating_changes(r1, RD1, sigma1, r2, RD2, sigma2, s, value=1): # Here s is the score of the first player against the second
    ...
    phi1_ = math.sqrt(phi1**2+find_new_sigma(sigma1, delta1, phi1, v1)**2)
    phi2_ = math.sqrt(phi2**2+find_new_sigma(sigma2, delta2, phi2, v2)**2)

    phi1__ = 1/math.sqrt(1/phi1_**2+1/v1)
    phi2__ = 1/math.sqrt(1/phi2_**2+1/v2)

    mu1__ = mu1 + phi1__**2*g(phi2)*(s - E(mu1, mu2, phi2)) * value
    mu2__ = mu2 + phi2__**2*g(phi1)*((1-s) - E(mu2, mu1, phi1)) * value
